...
fs.createReadStream(csvFileName)
  ...
  .on("data", async (row: any) => {
    const applicant : any = {
      ...
      university: {
        connect: {
          name: row["University or organization you belong to"].trim() as string
        }
      },
...
async function upsertUniversity(university : string){
  await prisma.$transaction(async (tx) => {
    ...
    .catch(async (e :any) => {
      if (e instanceof PrismaClientKnownRequestError && e.code == "P2002"){ // Retry
        /* See https://www.prisma.io/docs/reference/api-reference/prisma-client-reference#unique-key-constraint-errors-on-upserts */
        console.warn("Failed university upsert: "+university+". CODE P2002. RETRYING")
        await upsertUniversity(university);
      } else {
...
async function upsertApplicant(applicant : any){
  ...
  await prisma.$transaction(async (tx) => {
    ...
    .catch(async (e:any) => {
      if (e instanceof PrismaClientKnownRequestError && e.code == "P2002"){ // Retry
        /* See https://www.prisma.io/docs/reference/api-reference/prisma-client-reference#unique-key-constraint-errors-on-upserts */
        console.warn("Failed applicant upsert: " + applicant.fullName + ". CODE P2002. RETRYING")
        await upsertApplicant(applicant);
      } else {
