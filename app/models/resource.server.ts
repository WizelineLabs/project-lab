...
export async function getResourceProviders() {
    return db.resource.findMany({ select: { provider: true}, distinct: ['provider'] })
}
...
export async function getDistinctResources() {
    const promises = [
        db.resource.findMany({ select: { type: true, provider: true, name: true }, distinct: ['type'] }),
        db.resource.findMany({ select: { type: true, provider: true, name: true }, distinct: ['provider'] }),
        db.resource.findMany({ select: { type: true, provider: true, name: true }, distinct: ['name'] }),
    ]
