    ...
    if(repo != ''){
        ...
        try{
            ...
            if(repoReleases.status != 404){
                ...
                repoReleases.data?.forEach( (data: { id: string; body: string; name: string; tag_name: string; author: { login: string; }; prerelease: boolean; published_at: string; html_url: string; }) => {
                    saveRelease(data.id.toString(), data.body, data.name , data.tag_name, data.author?.login, data.prerelease, data.published_at, projectId, data.html_url, db)
                return;
                });
            }
