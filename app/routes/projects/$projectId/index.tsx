...
export const validateRPStructure = (stringData: string) => {
  const vals = JSON.parse(stringData)
  const validKeys = vals.find(
    (val: object) => val.hasOwnProperty("id") && val.hasOwnProperty("name")
  )
  return !!(vals.length === 0 || validKeys)
}

export const validatorRelatedProjects = withZod(
  zfd
    .formData({
      relatedProjects: z.string(),
    })
    .refine(({ relatedProjects }) => validateRPStructure(relatedProjects), {
  ...
  try {
    switch (action) {
      ...
      case "EDIT_RELATED_PROJECTS":
        const result = await validatorRelatedProjects.validate(form)
        if (result.error) return validationError(result.error)
        const { submittedData } = result
        const relProProjectId: string = submittedData.projectId
          ? submittedData.projectId
          : ("" as string)
        const relatedProjectsParse: Array<any> = submittedData.relatedProjects
          ? JSON.parse(submittedData.relatedProjects as string)
          : []
...
export default function ProjectDetailsPage() {
  ...
  return (
    <>
      ...
      <div className="wrapper">
        <ValidatedForm
          validator={validatorRelatedProjects}
          defaultValues={{
            relatedProjects: project.relatedProjects ? JSON.stringify(project.relatedProjects) : '',
          }}
          method="post"
          subaction="saveRelatedProjects"
        >
