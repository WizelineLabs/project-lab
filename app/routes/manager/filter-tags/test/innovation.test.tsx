...
describe("Innovation Tiers test", () => {
  ...
  test("Path loader", async () => {
    let request = new Request("http://localhost:3000/manager/filter-tags/innovation-tiers")
  ...
  test("Delete button opens the delete modal", async () => {
    ...
    render(<InnovationTiers />)
    expect(await screen.findByTestId("testInnovationDelete")).toBeInTheDocument()
  ...
  test("Create button adds a new row in the table", async () => {
    ...
    render(<InnovationTiers />)
    expect(await screen.findByTestId("testInnovationCreate")).toBeInTheDocument()
